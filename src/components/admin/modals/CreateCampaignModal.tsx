...
interface CreateCampaignModalProps {
  ...
  setNewCampaignDesc: (val: string) => void;
  isUsersDropdownOpen: boolean;
  setIsUsersDropdownOpen: (val: boolean) => void;
  ...
  setNewTotalMonths: (val: string) => void;
  newWinnersPerDraw: string;
  setNewWinnersPerDraw: (val: string) => void;
  processing: boolean;
  panels?: DashboardPanel[];
  panelId?: string;
  setPanelId?: (val: string) => void;
...
export default function CreateCampaignModal({
  ...
  const filteredPanelUsers = React.useMemo(() => {
    return availableUsers.filter(u => {
      ...
      const targetPanel = panelId || 'default';
  ...
  }, [availableUsers, panelId]);

  React.useEffect(() => {
    const winners = Number(newWinnersPerDraw) || 1;
    if (selectedUsers.length > 0 && winners >= 1) {
      const computed = Math.ceil(selectedUsers.length / winners);
      setNewTotalMonths(String(computed));
    }
  }, [selectedUsers.length, newWinnersPerDraw, setNewTotalMonths]);

  // Sync selected list on panel selection changes to prevent mixed panels
  React.useEffect(() => {
    const validSelected = selectedUsers.filter(uid => {
    ...
    if (validSelected.length !== selectedUsers.length) {
      setSelectedUsers(validSelected);
    }
  }, [panelId, availableUsers, selectedUsers, setSelectedUsers]);
        ...
        <div className="fixed inset-0 bg-black/65 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <form onSubmit={handleCreateCampaign} className="space-y-4">
              {/* SECTION 1: Top info */}
              <div>
                <h5 className="font-bold font-sans text-sm mb-2.5">Create New Campaign</h5>
                    ...
                    {panels.length > 0 && (
                      <div>
                        ...
                        <select
                          ...
                          value={panelId}
                          onChange={(e) => setPanelId && setPanelId(e.target.value)}
              ...
              <div className={`p-3 rounded-xl border grid grid-cols-[1fr_1fr_auto] gap-3 items-end mt-2 ${
                ...
                <div className="flex flex-col text-right pb-1">
                  ...
                  <span className="text-xl font-bold text-[#F59E0B]">
                    {selectedUsers.length * (Number(newMonthlyAmount) || 0)} ৳
                  </span>
                </div>
              </div>
